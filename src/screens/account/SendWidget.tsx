...
const SendWidget = ({fetchData}: SendWidgetProps) => {
  ...
  return (
    <View style={[s.container, l.wide80, l.between]}>
      <Input label="To Address" value={toAddress} handleChange={setToAddress} />
      <Input
        label="Amount"
        value={amountToSend}
      ...
      <Button
        ...
        onPress={async () => {
          const response = await fetch(
            `http://jobcoin.gemini.com/skyline-departed/api/transactions?fromAddress=${user}&toAddress=${toAddress}&amount=${amountToSend}`,
            {method: 'POST'},
          );
